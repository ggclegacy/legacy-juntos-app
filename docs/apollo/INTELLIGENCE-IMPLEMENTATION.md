...
Production build, TypeScript and lint passed. 105 unit/API/database tests passed across runs; the existing database suite needed an isolated rerun after a startup timeout during concurrent machine activity. One live-model test was skipped. New tests exercise source quote grounding, malformed embedding rejection, provider consent, account changes during indexing, private/shared vector isolation, stale-vector invalidation, a synthetic archive and conversation-link authorization. Browser checks cover Apollo identity/privacy, teaching/correction/deletion and the new consent controls on desktop and mobile. The browser tests use the clearly labeled sample account; they do not establish live cloud readiness.
