...
import { payloadSchema } from "@/lib/protocols/model";
import { z } from "zod";
export async function GET(request: Request) {
  try {
    ...
    if (error)
      throw new ApiError(
        503,
        "Private health storage is unavailable. Apply migration 003.",
      );
...
export async function POST(request: Request) {
  try {
    ...
    if (input.payload.kind === "dose") {
      const p = input.payload;
      const { data } = await db
        .from("health_events")
        .select("payload")
        .eq("entity_id", p.protocolId)
        .eq("revision", p.protocolRevision)
        .eq("owner_id", user.id)
        .eq("workspace_id", membership.workspace_id)
        .single();
      if (
        !data ||
        data.payload.kind !== "protocol" ||
        data.payload.name !== p.name ||
        data.payload.instructions !== p.instructions
      )
        throw new ApiError(
          400,
          "The original protocol could not be verified. Reload before logging.",
        );
    }
    const { data, error } = await db
      .from("health_events")
